...
export function RsvpSection({ invitation, guest, variant = "classic" }: { invitation: Invitation; guest: Guest | null | undefined; variant?: Variant }) {
  const [step, setStep] = useState(0); // 0: code entry, 1: status, 2: details
    ...
    return (
      <SectionWrapper>
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="space-y-4">
          <div className="w-16 h-16 mx-auto rounded-full flex items-center justify-center" style={{ background: "var(--inv-primary)" }}>
            <Check className="w-8 h-8" style={{ color: "var(--inv-secondary)" }} />
          </div>
          <h2 className="text-3xl" style={{ fontFamily: "var(--inv-font-title)", color: "var(--inv-text)" }}>Thank You!</h2>
          <p style={{ color: "var(--inv-text-secondary)" }}>Your response has been recorded.</p>
        </motion.div>
      </SectionWrapper>
    );
  }
  ...
  return (
    <SectionWrapper>
      ...
      <AnimatePresence mode="wait">
        {step === 0 && !foundGuest && (
          <motion.div key="code" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4 max-w-xs mx-auto">
            <p className="text-sm" style={{ color: "var(--inv-text-secondary)" }}>Enter your invitation code</p>
            <input
              value={code}
            ...
            <button
              ...
              className="w-full px-6 py-3 rounded-lg font-medium disabled:opacity-50 transition-opacity"
              style={btnStyle}
            >
        ...
        {step === 1 && (
          <motion.div key="status" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
            <p className="text-lg font-medium" style={{ color: "var(--inv-text)" }}>Hi, {foundGuest?.full_name}!</p>
            <p className="text-sm" style={{ color: "var(--inv-text-secondary)" }}>Will you be attending?</p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              {([
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-6 py-3 rounded-lg border-2 font-medium transition-all hover:opacity-80"
                  style={{
        ...
        {step === 2 && (
          <motion.div key="details" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4 max-w-sm mx-auto">
            {status === "attending" && (foundGuest?.max_companions ?? 0) > 0 && (
              <div>
                <label className="block text-sm mb-1 text-left" style={{ color: "var(--inv-text-secondary)" }}>Number of companions (max {foundGuest?.max_companions})</label>
                <input
                  type="number"
                  min={0}
            ...
            <div>
              <label className="block text-sm mb-1 text-left" style={{ color: "var(--inv-text-secondary)" }}>Message for the host (optional)</label>
              <textarea
                value={message}
            ...
            {status === "attending" && (
              <div>
                <label className="block text-sm mb-1 text-left" style={{ color: "var(--inv-text-secondary)" }}>Dietary restrictions (optional)</label>
                <input
                  value={dietary}
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep(1)} className="px-6 py-3 rounded-lg border font-medium" style={{ borderColor: "var(--inv-accent)", color: "var(--inv-text)" }}>
                Back
              </button>
              <button
                onClick={submitRsvp}
